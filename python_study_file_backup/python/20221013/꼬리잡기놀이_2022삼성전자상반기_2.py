...
while turn <= k:
    ...
    test_ball_map = [[0 for _ in range(n)] for _ in range(n)]
    ...
    for i in range(n):
        ...
        test_ball_map[ny][nx] = i
        
        if 1 <= game_map[ny][nx] <= 3:
            print('game_map[ny][nx]??', game_map[ny][nx])
            for gii, g in enumerate(groups):
                if end_group[gii] == 1: # 그룹 중복체크 방지
                    continue
                print('gii, end_group[gii]??', end_group[gii])
