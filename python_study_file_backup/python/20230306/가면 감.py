...
def solution(reply):
    ...
    def remove_cycle():
        ...
        s_f, s_r = forward[n], reverse[n] # n의 forward, reverse 집합 값으로 초기화(동일주소)
        ...
        print('[1] cycles:{}, s_f:{}, s_r:{}'.format(cycles, s_f, s_r))
        ...
        for i in s_r:
            ...
            forward[i] -= cycles
            forward[i].add(n) # 이건 어떤 역할을 할까?
            print('after forward[i]:{}'.format(forward[i]))
        
        print('[2] cycles:{}, s_f:{}, s_r:{}'.format(cycles, s_f, s_r))
        print('forward:{} \nreverse:{}'.format(forward, reverse))
        ...
        s_f -= cycles
        s_r -= cycles
        print('[3] remaining:{}, s_f:{}, s_r:{}'.format(remaining, s_f, s_r))
        print('forward:{} \nreverse:{}'.format(forward, reverse))
        
        # forward[n]에 값이 남아있는 경우. remainig 재탐색.
        if s_f:
            return []
        # forward[n]에 값이 없는 경우. 해당 cycle 탐색을 멈추고 leafs로 remaining 삭제
        else:
            return [n]
        
    # old-본인이 원하는 사람이 없는 동아리원 확인
    # old-근데 문제 조건 상 이런 경우가 없는거 아닌가..
    # 반복문 진행하면서 필요함!
    # cycle 하나를 완전히 끝내고 관련 인원 삭제하기 위함
    def remove_leafs(leafs):
        ...
        while leafs:
            leaf = leafs.pop()
            print('leaf:{}, reverse[leaf]:{}'.format(leaf, reverse[leaf]))
            # old-본인은 원하는 사람이 없는데, 본인을 원하는 사람이 있는 경우
            # 연관있는 reverse 모두 삭제
            for p in reverse[leaf]:
                if p not in to_remove: # to_remove에 이미 속한 사람인지 확인
                    ...
                    leafs.append(p)
                    to_remove.add(p)
                    print('p:{}, to_remove:{}, leafs:{}'.format(p, to_remove, leafs))
