...
def wall_check(ny, nx, di, di2, wall):
    ...
    for i,w in enumerate(wall):
        #print('Turn {} w:{}'.format(i,w))
        if (w[2] == 0 and (di2 == 2 or di2 == 3)) or (w[2] == 1 and (di2 == 0 or di2 == 1)):
            if w[0] == wy and w[1] == wx:
                #print('False??')
                return False
        ...
        for x in range(C):
            ...
            if new_room_map[y][x] > 0:
                for i in range(4):
                    ...
                    if ny>=0 and ny<R and nx>=0 and nx<C :
                        if wall_check2(ny, nx, i, wall) and new_room_map[y][x] > new_room_map[ny][nx]:
