...
min_move_cnt = 13
def search_route(curr_pos, move_cnt, last_coin, coin_cnt):
    global min_move_cnt
        ...
        if in_range(nx,ny) and visited[nx][ny] < 2:
            ...
            if coin_map[nx][ny] not in (0,-1,-2):
                if coin_map[nx][ny] > last_coin:
                    # 동전 줍기
                    visited[nx][ny] += 1
                    search_route((nx,ny), move_cnt+1, coin_map[nx][ny], coin_cnt+1)
                    visited[nx][ny] -= 1
                
            # 동전 안줍기
            visited[nx][ny] += 1
            search_route((nx,ny), move_cnt+1, last_coin, coin_cnt)
            visited[nx][ny] -= 1


visited[start_pos[0]][start_pos[1]] = 1
search_route(start_pos, 0, 0, 0)
