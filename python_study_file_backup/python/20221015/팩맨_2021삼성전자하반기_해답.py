...
def simulate():
	...
	for i in range(n):
		for j in range(n):
			for k in range(DIR_NUM):
				if monster[t_num-1][i][j][k]:
					print(t_num-1,' y,x,k? ', i,j,k, ' 개수? ', monster[t_num-1][i][j][k])
	print('\n')
	...
	for i in range(n):
		for j in range(n):
			for k in range(DIR_NUM):
				if monster[t_num][i][j][k]:
					print(t_num,' y,x,k? ', i,j,k, ' 개수? ', monster[t_num][i][j][k])
			for k in range(MAX_DECAY):
				if dead[i][j][k]:
					print('dead? y,x,k? ', i,j,k)
	print('\n')
